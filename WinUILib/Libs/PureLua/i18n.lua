...
-- Source: kikito/i18n.lua 0.9.2 (single-file adaptation for WoW)
local function __MetroLib_Load()
  local unpack = unpack or table.unpack

  local function __build_plural_module()
    local plural = {}
  ...
  local function __build_variants_module()
    local variants = {}
  ...
  local function __build_interpolate_module()
  ...
  local version = '0.9.2'
  local plural = __build_plural_module()
  local interpolate = __build_interpolate_module()
  local variants = __build_variants_module()
  ...
  local function createTranslator(seedStore, seedLocale, seedFallback)
    local store = seedStore or {}
    local locale
    local pluralizeFunction
    local defaultLocale = 'en'
    local fallbackLocale = seedFallback or defaultLocale
    ...
    local function syncPublic()
      ...
      translator.fallbackLocale = fallbackLocale
    end
    ...
    function translator.set(key, value)
    ...
    function translator.setLocale(newLocale, newPluralizeFunction)
    ...
    function translator.reset()
      store = {}
      plural.reset()
      translator.setLocale(seedLocale or defaultLocale)
      translator.setFallbackLocale(seedFallback or seedLocale or defaultLocale)
      syncPublic()
    end

    function translator.load(data)
      recursiveLoad(nil, data)
      syncPublic()
    end

    function translator.loadFile(_)
      return nil, 'loadFile unsupported in WoW embedding'
    end

    setmetatable(translator, { __call = function(_, ...) return translator.translate(...) end })
    translator.reset()
    if seedStore and next(seedStore) ~= nil then
      translator.load(seedStore)
    end
    return translator
  end

  local i18n = createTranslator(nil, 'en', 'en')
  i18n.version = version
  i18n._VERSION = version
  i18n.plural = plural
  i18n.interpolate = interpolate
  i18n.variants = variants
  i18n.new = function(seed, loc, fallback)
    return createTranslator(seed, loc or 'en', fallback or loc or 'en')
  end
