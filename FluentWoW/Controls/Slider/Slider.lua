...
function SliderMixin:_RefreshTicks()
    if not self._tickPool then
        self._tickPool = lib.FramePool:New("Frame", self.TickContainer, "FWoWSliderTickTemplate")
    end
...
function FWoWSlider_OnLoad(self)
...
function FWoWSlider_OnEnter(self)
    local parent = self:GetParent()
...
function FWoWSlider_OnLeave(self)
    local parent = self:GetParent()
    if not parent._enabled then return end
...
function FWoWSlider_OnMouseDown(self)
    local parent = self:GetParent()
    if not parent._enabled then return end
...
function lib:CreateSlider(parent, name)
    local f = CreateFrame("Frame", name, parent, "FWoWSliderTemplate")
    ---@cast f FWoWSlider
    return f
end
