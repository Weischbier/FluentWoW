...
function ComboMixin:_BuildDropdown()
    if not self._itemPool then
        self._itemPool = lib.FramePool:New("Button", self.Dropdown.Scroll.Child, "FWoWComboBoxItemTemplate", function(btn)
...
function FWoWComboBox_OnLoad(self)
...
function FWoWComboBox_OnClick(self)
...
function FWoWComboBox_OnEnter(self)
    if not self._enabled then return end
    if self._vsm:GetState() ~= "Expanded" then
...
function FWoWComboBox_OnLeave(self)
    if not self._enabled then return end
    if self._vsm:GetState() ~= "Expanded" then
        self._vsm:SetState("Normal")
    end
...
function FWoWComboBoxItem_OnClick(self)
...
function FWoWComboBoxItem_OnLeave(self)
    self.Highlight:Hide()
end
...
function lib:CreateComboBox(parent, name)
    local f = CreateFrame("Frame", name, parent, "FWoWComboBoxTemplate")
    ---@cast f FWoWComboBox
    return f
end
